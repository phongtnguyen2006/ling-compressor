...
_STACK_RE = re.compile(r'^\s*(File \"|Traceback|at\s+\S+\()')
...
def _find_table_line_indices(lines: list[str]) -> set[int]:
    table_lines: set[int] = set()
    for idx, line in enumerate(lines):
        if not _TABLE_SEP_RE.match(line):
            continue
        table_lines.add(idx)
        # header row immediately preceding the separator, if it looks like one
        prev_idx = idx - 1
        if prev_idx >= 0 and "|" in lines[prev_idx]:
            table_lines.add(prev_idx)
        # data rows following the separator, while they contain "|"
        j = idx + 1
...
def _classify_line(
    line: str, lines: list[str], idx: int, table_lines: set[int]
) -> BlockKind:
...
def segment(text: str) -> list[Block]:
    ...
    table_lines = _find_table_line_indices(lines)
    ...
    for idx, line in enumerate(lines):
        ...
        if is_fence_delim:
            ...
            kinds.append(BlockKind.PASSTHROUGH)
            continue
        kinds.append(_classify_line(line, lines, idx, table_lines))
